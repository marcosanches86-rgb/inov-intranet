...
//    AuthMiddleware::class          → requires authenticated session
//    RoleMiddleware::role('admin')  → requires admin or above
//    CsrfMiddleware::class          → validates CSRF token
//
//  Built progressively per phase:
//    Phase 2 → health check (this file)
//    Phase 4 → auth routes
//    Phase 5 → users + companies
//    Phase 6 → news + announcements
//    Phase 7 → documents + brands + gallery
...
$router->post(  '/api/documents',               [DocumentController::class, 'store'],    [AuthMiddleware::class, RoleMiddleware::role('editor')]);
...
$router->post(  '/api/brand-assets',      [BrandAssetController::class, 'store'],   [AuthMiddleware::class, RoleMiddleware::role('editor')]);
...
$router->post(  '/api/gallery/{id}/items',          [GalleryController::class, 'addItem'],    [AuthMiddleware::class, RoleMiddleware::role('editor')]);
...
// aliases com plural (compatibilidade)
...
$router->get('/api/admin/dashboard', [DashboardController::class, 'index'], [AuthMiddleware::class, RoleMiddleware::role('admin')]);
$router->get('/api/admin/logs',      [DashboardController::class, 'logs'],  [AuthMiddleware::class, RoleMiddleware::role('admin')]);
