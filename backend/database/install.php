<?php
// ================================================================
//  INOV INTRANET — One-time DB setup script
//  DELETE THIS FILE AFTER RUNNING!
// ================================================================
$secret = $_GET['key'] ?? '';
if ($secret !== 'inov-setup-2025') { die('Forbidden'); }
...
foreach (array_filter(array_map('trim', explode(";\n", $sql))) as $stmt) {
    ...
    try {
        $pdo->exec($stmt . ';');
        $first50 = substr(preg_replace('/\s+/', ' ', $stmt), 0, 50);
        $ok[] = $first50;
    } catch (Exception $e) {
        $errors[] = $e->getMessage();
    }
...
echo '<h2>INOV Intranet — Setup Result</h2>';
...
echo '<p><strong>DELETE THIS FILE!</strong> Access: /backend/database/install.php</p>';
